...
var employee={
    ...
    addEmployee:function(item,callback){
            return db.query("insert into employee_tbl values(?,?,?,?,?,?,?,?,?,?,?)",[item.employee_email,item.employee_password,item.employee_name,item.employee_joining_date,item.employee_salary,item.employee_designation,item.employee_mobileno,item.employee_city,item.employee_gender,item.employee_address,item.employee_type],callback);        
        },
